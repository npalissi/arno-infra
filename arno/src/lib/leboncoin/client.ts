...
function buildSearchPayload(params: LeboncoinSearchParams) {
  ...
  // Brand+model as text keywords — LBC enum brand doesn't work for all brands
  // (e.g. "BMW" returns 0 with enum but 22000+ with text)
...
enums = {"ad_type": ["offer"]}
# Brand+model as text (enum brand doesn't work for all brands e.g. BMW)
